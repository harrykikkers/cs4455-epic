...
string CryptoHelpers::toBase64(const vector<unsigned char>& data) {
...
vector<unsigned char> CryptoHelpers::fromBase64(const string& text) {
    vector<unsigned char> decoded(text.size());
    size_t decodedLen = 0;
...
string CryptoHelpers::generateBase64Nonce() {
...
string CryptoHelpers::generateKeypairPublicBase64(string& outSecretKeyBase64) {
    vector<unsigned char> publicKey(crypto_box_PUBLICKEYBYTES);
    vector<unsigned char> secretKey(crypto_box_SECRETKEYBYTES);
...
string CryptoHelpers::encryptMessage(const string& plaintext,
...
string CryptoHelpers::decryptMessage(const string& ciphertextBase64,
    ...
    vector<unsigned char> plaintext(ciphertext.size() - crypto_box_MACBYTES);
